...
public class CNRGMT_PianoRate_InserimentoPianoRate extends MbJavaComputeNode {
	...
	public void evaluate(MbMessageAssembly inAssembly) throws MbException {
		...
		try {
			...
			String sel = "Select * from "+ UserCNRG + ".B2077_TO_IN_PIANO_RATE where AUDIT_DATA_ELAB_DWH is null";
				...
				int nr = rsmd.getColumnCount();
		         ...
		         for(int i=1; i<=nr; i++){
			       	 if (rs.getString(i) != null && !rsmd.getColumnLabel(i).equals("DATA_CARICAMENTO") && !rsmd.getColumnLabel(i).equals("EXTERNAL_ID") && !rsmd.getColumnLabel(i).equals("STATO") 
			       			 	&& !rsmd.getColumnLabel(i).equals("DATA_LAVORAZIONE")) 
			       	 {
		    			...
		    			} else if (C1.equals("ID_SISTEMA_PROVENIENZA_DATI") ||C1.equals("ID_SOCIETA_COMPETENZA") || C1.equals("ID_FATTURA")|| C1.equals("ID_RATA") ){
		    				ValueIns = ValueIns.concat(V1).concat("','");
		    			...
		    			Insert = "INSERT INTO STAGING_DWH.CNRGLM_IN_PIANO_RATE ("+ColonneIns+") VALUES ('" + ValueIns +")";
		    			...
		    			Insert = "UPDATE STAGING_DWH.CNRGLM_IN_PIANO_RATE  SET "+ValueUpdate+" WHERE " + ValueQuery +"";
						...
						String Update = "UPDATE "+ UserCNRG + ".B2077_TO_IN_PIANO_RATE SET AUDIT_DATA_ELAB_DWH = to_date (sysdate)  where " + UPPQuery;
			...
			String SemaforoUP = "UPDATE STAGING_DWH.CNRGLM_SEMAFORO SET STATO_ELABORAZIONE = 1, TS_FINE = SYSDATE WHERE NOME_TABELLA = 'CNRGLM_IN_PIANO_RATE' and STATO_ELABORAZIONE = 0 and TIPO_ELABORAZIONE = 'C'";
